...
# Modelin Görselleştirilmesi (Visualization of the Model)
...
# Cost function
def cost_function(Y, b, w, x):  # Y: bağımlı değ.  b: sabit  w: ağırlık  X: bağımsız değişken
    m = len(Y)
    sse = 0
...
# update_weights
def update_weigths(Y, b, w, X, learning_rate):      # learning_rate: öğrenme hızı
    m = len(Y)
